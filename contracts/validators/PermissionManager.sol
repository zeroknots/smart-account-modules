 ...
 *         without calling onUninstall on every sub-module. Probably need to introduce same nonce system for submodules
 * as well
 ...
 *         but how do we get all the submodules used for given SA, while indexing is by SignerIds. the only way is
 * storing
...
contract PermissionManager is ERC7579ValidatorBase, ERC7579ExecutorBase, IPermissionManager {
    ...
    bytes32 constant PERMISSION_VALIDATOR_STORAGE_SLOT =
        0x73a9885e8be4b58095971868aa2af983b5913f3e08c5b78a3ca0cb6b827458f8;
    ...
    struct PermissionValidatorStorage {
        ...
        // - multisig signer: keccak256(smartAccount.address + validationAlgo.address + ownersAddresses + number of
        // owners)
        // - etc
        ...
        returns (ValidationData vd)
    {
         ...
         */
        if (
            ISignerValidator(signerValidator).checkSignature(
                SignerId.unwrap(signerId), msg.sender, userOpHash, cleanSig
            ) == EIP1271_FAILED
        ) {
        ...
        returns (bytes4 sigValidationResult)
    {
        ...
        SignerId signerId =
            SignerId.wrap((bytes32(signature[1:33])).mixinNonce(_permissionValidatorStorage().nonces[msg.sender]));
        ...
        if (signerValidator != NO_SIGNATURE_VALIDATION_REQUIRED) {
            ...
            if (
                ISignerValidator(signerValidator).checkSignature(SignerId.unwrap(signerId), msg.sender, hash, cleanSig)
                    == EIP1271_FAILED
            ) {
                return EIP1271_FAILED;
            }
        ...
        returns (SignerId, bytes calldata, address)
    {
        ...
        // check chainId
        // make permissionDataDigest from permissionData and compare with permissionDataDigest obtained from
        // permissionEnableData
        {
            (uint64 permissionChainId, bytes32 permissionDigest) =
                _parsePermissionFromPermissionEnableData(permissionEnableData, permissionIndex);
    ...
    {
        if (
            IERC7579Account(smartAccount).isValidSignature(permissionEnableDataHash, permissionEnableDataSignature)
                != EIP1271_SUCCESS
        ) {
            ...
            for (uint256 i; i < executions.length; i++) {
                vd = vd.intersectValidationData(
                    _validateSingleExecution(
                        signerId, executions[i].target, executions[i].value, executions[i].callData, userOp
                    )
                );
            }
        ...
        returns (ValidationData vd)
    {
        ...
        vd = _validateActionPolicies(
            keccak256(abi.encodePacked(signerId, actionId)), policies, target, value, data, userOp
        );
    }
                        ...
                        bytes32(
                            _safeCallSubmoduleViaTrustedForwarder(
                                ...
                                abi.encodeWithSelector(
                                    IUserOpPolicy.checkUserOp.selector, SignerId.unwrap(signerId), userOp
                                )
                            )
                        )
                    )
                    ...
                    uint256(
                        bytes32(
                            _safeCallSubmoduleViaTrustedForwarder(
                                policies.get(i),
                                abi.encodeWithSelector(
                                    IActionPolicy.checkAction.selector, id, target, value, data, userOp
                                )
                            )
                        )
                    )
    ...
    function _safeCallSubmoduleViaTrustedForwarder(
        address submodule,
        bytes memory data
    )
        internal
    ...
    function _clean7579ExecutionCalldata(bytes calldata userOpCallData)
        internal
        pure
    ...
    function _addPolicy(AddressArray storage policies, address policy) internal {
        if (!policies.contains(policy)) {
            ...
            //console2.log("check from storage: ",
            // _permissionValidatorStorage().userOpPolicies[signerId][smartAccount].data[policies.lastUsedIndex()]);
        } else {
            // same policy can not be used twice as the policy of the sane type (userOp, action, 1271)
            // for the same id and smartAccount, as inside the policy contract the config is stored as
            // id=>smartAccount=>config
            // so same policy can be used as 1271 and userOp and action for the same SA, as ids will be different
            // also same policy can be used several times as action policy for the same signerId and SA, as soon as it
            // is used
        ...
        internal
    {
        try IERC165(subModule).supportsInterface(type(ITrustedForwarder).interfaceId) returns (bool supported) {
            ...
            _execute(
                smartAccount, subModule, 0, abi.encodeWithSelector(IERC7579Module.onInstall.selector, subModuleInitData)
            );
        }
    }
    ...
    function _decodeEnableModeUserOpSignature(bytes calldata signature)
        internal
        pure
        returns (
            uint8 permissionIndex,
    ...
    function _enablePermission(bytes calldata permissionData) internal returns (SignerId, address) {
        SignerId signerId =
            SignerId.wrap(bytes32(permissionData[0:32]).mixinNonce(_permissionValidatorStorage().nonces[msg.sender]));
    ...
    function _parseAndEnableSigner(
        SignerId signerId,
        bytes calldata permissionData
    )
        internal
    ...
    function _parsePolicy(bytes calldata partialPermissionData)
        internal
        pure
    ...
    function renounceUserOpPolicy(SignerId signerId, address policy) external {
        ...
        _safeCallSubmoduleViaTrustedForwarder(
            policy,
            abi.encodeWithSelector(IERC7579Module.onUninstall.selector, abi.encodePacked(SignerId.unwrap(signerId)))
        );
    }
